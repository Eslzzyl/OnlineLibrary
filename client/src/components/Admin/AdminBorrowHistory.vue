<template>
  <v-container>
    <v-banner sticky style="font-size: 2rem;">借阅历史</v-banner>
...
async function request(page, itemsPerPage, sortBy, search) {
  ...
  const url = `/book/borrowhistory?pageIndex=${page - 1}&pageSize=${itemsPerPage}&sortColumn=${orderBy}&sortOrder=${order}&filterQuery=${search}`
  try {
